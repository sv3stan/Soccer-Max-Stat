...
procedure TForm2.Button1Click(Sender: TObject);
  ...
  begin
    ...
    EasyQuery2_F2.Active := false;
    EasyQuery2_F2.SQL.clear;
    EasyQuery2_F2.SQL.add
        ('select Tur, Data, Team_1, Goal_F_1, Goal_F_2, (Goal_T_1-Goal_F_1) as Goal_S_1, (Goal_T_2-Goal_F_2) as Goal_S_2, Goal_T_1, Goal_T_2, Team_2, Referi, Location, Fans from play where (status=10) and ((Team_1=:Team) or (Team_2=:Team))');
    EasyQuery2_F2.ParamByName('Team').AsString := NxComboBox1.Text;
    EasyQuery2_F2.Active                       := true;
    EasyQuery2_F2.Open;
    EasyQuery2_F2.first;

    // ShortDateFormat := 'dd/mm';
    For i := 0 to EasyQuery2_F2.RecordCount - 1 do
      begin
        Form1.NextGrid13.AddRow(1);
        Form1.NextGrid13.Cells[0, i] := EasyQuery2_F2.Fields[0].AsString; // Тур
        Form1.NextGrid13.Cells[1, i] := EasyQuery2_F2.Fields[1].AsString;
        // Дата
        Form1.NextGrid13.Cells[2, i] := EasyQuery2_F2.Fields[2].AsString;
        // Первая команда
        Form1.NextGrid13.Cells[3, i] := EasyQuery2_F2.Fields[3].AsString + ':' +
            EasyQuery2_F2.Fields[4].AsString; // Первый тайм
        Form1.NextGrid13.Cells[4, i] := EasyQuery2_F2.Fields[5].AsString + ':' +
            EasyQuery2_F2.Fields[6].AsString; // Второй тайм
        Form1.NextGrid13.Cells[5, i] := EasyQuery2_F2.Fields[7].AsString + ':' +
            EasyQuery2_F2.Fields[8].AsString; // Итог
        Form1.NextGrid13.Cells[6, i] := EasyQuery2_F2.Fields[9].AsString;
        // Вторая команда
        Form1.NextGrid13.Cells[7, i] := EasyQuery2_F2.Fields[10].AsString;
        // Рефери

        EasyQuery1_F2.Active := false;
        EasyQuery1_F2.SQL.clear;
        EasyQuery1_F2.SQL.add
            ('select Stadion, StadiumCap from Location where ID=:Locate');
        EasyQuery1_F2.ParamByName('Locate').AsString := EasyQuery2_F2.Fields
            [11].AsString;
        EasyQuery1_F2.Active := true;
        EasyQuery1_F2.Open;
        EasyQuery1_F2.first;

        Form1.NextGrid13.Cells[8, i] := EasyQuery1_F2.Fields[0].AsString;
        // Стадион
        Form1.NextGrid13.Cells[9, i] := EasyQuery2_F2.Fields[12].AsString +
            ' из ' + EasyQuery1_F2.Fields[1].AsString; // Зрители

        if EasyQuery2_F2.Fields[2].AsString = NxComboBox1.Text then
          begin
            If EasyQuery2_F2.Fields[7].AsInteger > EasyQuery2_F2.Fields[8].AsInteger
            then
              begin
                for y := 0 to 9 do
                  begin
                    Form1.NextGrid13.Cell[y, i].color      := Form1.Cl_Win;
            ...
            then
              begin
                for y := 0 to 9 do
                  begin
                    Form1.NextGrid13.Cell[y, i].color      := Form1.Cl_Draw;
                    Form1.NextGrid13.Columns[y].Font.color := Form1.Cl_Text;
                  end;
              end;
            If EasyQuery2_F2.Fields[7].AsInteger < EasyQuery2_F2.Fields[8].AsInteger
            then
              begin
                for y := 0 to 9 do
                  begin
                    Form1.NextGrid13.Cell[y, i].color      := Form1.Cl_Loss;
                    Form1.NextGrid13.Columns[y].Font.color := Form1.Cl_Text;
                  end;
              end;
          end;

        if EasyQuery2_F2.Fields[9].AsString = NxComboBox1.Text then
          begin
            If EasyQuery2_F2.Fields[7].AsInteger < EasyQuery2_F2.Fields[8].AsInteger
            then
              begin
...
procedure TForm2.FormActivate(Sender: TObject);
  ...
  begin
    ...
    EasyQuery1_F2.SQL.clear;
    EasyQuery1_F2.SQL.add
        ('select ShortName from location order by ShortName asc');
    EasyQuery1_F2.Active := true;
    EasyQuery1_F2.Open;
    EasyQuery1_F2.first;
    NxComboBox1.Text := EasyQuery1_F2.Fields[0].AsString;
    NxComboBox2.Text := EasyQuery1_F2.Fields[0].AsString;
    for i            := 0 to EasyQuery1_F2.RecordCount - 1 do
      begin
        NxComboBox1.Items.add(EasyQuery1_F2.Fields[0].AsString);
        NxComboBox2.Items.add(EasyQuery1_F2.Fields[0].AsString);
        EasyQuery1_F2.Next;
      end;
    NxComboBox3.Text := '=0';
    NxComboBox4.Text := '=0';
